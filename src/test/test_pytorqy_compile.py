...
class TestPytorqyComile(unittest.TestCase):
    def test1st(self):
        compiling('text match= (v <- +(r"^\d" | ".")) | (null <- +(" " | ">"));')
        
    def test2nd(self):
        compiling('text match= (v <- (null <- "("), +(&0 | xcp("(" | ")"), any), (null <- ")"));')
    
    def test3rd(self):
        compiling('text match= ?(v <- (u_op <- "+" | "-"), (v scan= &0)), *(xcp(v), any, ?(v <- (u_op <- "+" | "-"), (v scan= &0)) | (v scan= &0));')
...
#        self.assertTrue(exprs == exprs2)
    ...
    def test9th(self):
        exprStr = "(hoge <-), $huga, ($boo match= a), ($foo scan= b);"
        compiling(exprStr)
